...
void loop(){
 ...
 //Serial.print(" | Angle  = "); Serial.println(angle_pitch_output);
  ...
  int xReading = angle_roll_output;   //map(angle_roll_output,-,,0,1024);
  int yReading = angle_pitch_output;  //map(angle_pitch_output,,,0,1024);

  
  int a, b, c, d;

  
    if (xReading > (yReading-2) && xReading < (yReading+2)) {
    a = 0; 
    }else a = xReading*(-1);

    if (yReading > (yReading-2) && yReading < (yReading+2)) {
    b = 0; 
    }else b = yReading*(-1);
